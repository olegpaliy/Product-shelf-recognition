...
TEXT_PROMPTS: Dict[str, List[str]] = {
    ...
    "bonaqua": ["a Bonaqua clear mineral water bottle"],
    "schweppes": ["a Schweppes tonic water can", "Schweppes soft drink can"],
    "lipton": ["a yellow Lipton iced tea can or bottle"],
    "monster": ["a Monster Energy drink can with green claw logo"],
    "red-bull": ["a blue and silver Red Bull energy drink can"],
    "morshynska": [
        "Morshynska Ukrainian mineral water bottle",
        "моршинська негазована water bottle with red label text",
    ],
    "voda": ["clear water bottle with vertical white VODA text label"],
    "juice": ["orange juice plastic bottle in a fridge"],
}
...
class BrandMatcher:
    ...
    def predict_crop(self, crop_bgr: np.ndarray) -> BrandPrediction:
        ...
        for brand in brands:
            ...
            if refs is not None:
                parts.append(float(np.max(refs @ emb)))
